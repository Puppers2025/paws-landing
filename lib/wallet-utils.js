...
export const detectWalletProvider = () => {
  ...
  if (window.ethereum) {
    ...
    // Check for Rabby by checking the provider name
    if (window.ethereum.providers) {
      const rabbyProvider = window.ethereum.providers.find(provider => 
        provider.isRabby || provider.constructor.name === 'RabbyProvider'
      );
      if (rabbyProvider) return 'Rabby Wallet';
    }
  ...
  if (window.rabby) return 'Rabby Wallet';
  if (window.ethereum?.isRabby) return 'Rabby Wallet';
  
  // Check user agent for Rabby
  if (typeof navigator !== 'undefined' && navigator.userAgent.includes('Rabby')) {
    return 'Rabby Wallet';
  }
