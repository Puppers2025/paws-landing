...
export class DiscordWebhookService {
  ...
  /**
   * Initialize Puppers Game Bot via Railway
   * Uses Railway Discord bots for native notification badges
   */
  async initializeGameBot() {
    // Check if Railway Discord service is available
    try {
      const response = await fetch('/api/railway-discord?action=status');
      const result = await response.json();
      return result.status === 'available';
    } catch (error) {
      console.warn('🎮 Railway Discord service not available:', error.message);
      return false;
    }
  }
  ...
  async postLevelUpNotification(user, levelUpData) {
    try {
      // Try to use Railway Discord bots for native notification badges
      const botAvailable = await this.initializeGameBot();
      
      if (botAvailable) {
        try {
          const response = await fetch('/api/railway-discord', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
            body: JSON.stringify({
              action: 'post-level-up',
              data: { user, levelUpData }
            })
          });
          ...
          if (response.ok) {
            const result = await response.json();
            if (result.success) {
              console.log(`🚂 Posted level up notification via Railway for ${user.username} (Level ${levelUpData.newLevel})`);
              return true;
            }
          }
        } catch (error) {
          console.warn('🚂 Railway Discord service error, falling back to webhook:', error.message);
        }
      }
      ...
      if (botAvailable) {
        try {
          const response = await fetch('/api/railway-discord', {
            ...
            body: JSON.stringify({
              action: 'post-role-change',
              data: { user, roleChangeData }
            })
          });
          ...
          if (response.ok) {
            const result = await response.json();
            if (result.success) {
              console.log(`🚂 Posted role change notification via Railway for ${user.username}`);
              return true;
            }
          }
        } catch (error) {
