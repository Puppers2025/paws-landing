 ...
 * Lightweight Discord integration using webhooks with optional Discord bot for native notification badges
 */
...
export class DiscordWebhookService {
  ...
  /**
   * Initialize Discord bot (only when needed and available)
   */
  async initializeBot() {
    if (!this.discordBot && process.env.DISCORD_BOT_TOKEN) {
      try {
        const { DiscordBot } = await import('./discord-bot.js');
        this.discordBot = new DiscordBot();
        return true;
      } catch (error) {
        console.warn('Discord bot not available in this environment:', error.message);
        return false;
      }
    }
  ...
  async postLevelUpNotification(user, levelUpData) {
    try {
      // Try to use Discord bot for native notification badges
      const botAvailable = await this.initializeBot();
      
      if (botAvailable && !this.botConnected) {
        this.botConnected = await this.discordBot.connect();
      }

      if (this.botConnected) {
        // Use Discord bot for native notification badges
        const success = await this.discordBot.postLevelUpNotification(user, levelUpData);
  ...
  async postRoleChangeNotification(user, roleChangeData) {
    try {
      ...
      if (this.botConnected) {
        // Use Discord bot for native notification badges
        const success = await this.discordBot.postRoleChangeNotification(user, roleChangeData);
  ...
  /**
   * Fallback webhook notification when Discord bot is not available
   * @param {Object} user - User object
